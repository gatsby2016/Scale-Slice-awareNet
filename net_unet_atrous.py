...
class AUNet(nn.Module):
    def __init__(self, in_channels=1, n_classes=2, feature_scale=1, is_deconv=True, is_batchnorm=True):
        ...
        self.aspp = _ASPP(filters[0], filters[0], [1, 2, 3, 5, 8],  norm_layer=nn.BatchNorm2d, norm_kwargs=None)
        # downsampling
    ...
    #     x = self.aspp(maxpool1) # 64*256*256
    #
    #     conv2 = self.conv2(x)  # 128*256*256
    ...
    #     maxpool4 = self.maxpool(conv4)  # 512*32*32
    #
    #     center = self.center(maxpool4)  # 1024*32*32
    #     up4 = self.up_concat4(center, conv4)  # 512*64*64
    #     up3 = self.up_concat3(up4, conv3)  # 256*128*128
    #     up2 = self.up_concat2(up3, conv2)  # 128*256*256
    #     up1 = self.up_concat1(up2, conv1)  # 64*512*512
    #
    #     final = self.final(up1)
    #     return final

    ### while major revision, i change to 3 pooling as the unet_atrous_center
    def forward(self, inputs):
        conv1 = self.conv1(inputs)  #output: 64*512*512
        x = self.aspp(conv1) # 64*256*256
        maxpool1 = self.maxpool(conv1)  # 64*256*256
        ...
        up2 = self.up_concat2(up3, conv2)  # 128*256*256
        up1 = self.up_concat1(up2, x)  # 64*512*512
