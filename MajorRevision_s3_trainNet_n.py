...
numslices = 7
root = '/home/cyyan/projects/ISICDM2019/data/' + str(numslices) + 'slices/'
train_loader, val_loader, train_nums, val_nums = majorRevision_reach_data(root, batch_size=3)

net = msacUNet(in_channels=1, n_classes=nums_class, feature_scale=2, nslices=numslices).cuda()
# net.load_state_dict(torch.load('/home/cyyan/projects/ISICDM2019/model/unet_2567.pkl'))
print(net)

criterion = nn.CrossEntropyLoss(weight=weights).cuda()
# criterion = DiceCEloss(classes=nums_class, samples=3)
...
for epoch in range(epoches):
    ...
    if val_mIOU > metric:
        metric = val_mIOU
        torch.save(net.state_dict(), '../model2D/%d_slices_msacunet_%d.pkl'%(numslices, int(metric*10000)))
