...
class msacUNet(nn.Module):
    def __init__(self, in_channels=1, n_classes=2, feature_scale=1, nslices=3, is_deconv=True, is_batchnorm=True):
        ...
        self.conv4 = unetConv2(filters[2]*self.nslices, filters[3], self.is_batchnorm)
        self.maxpool = nn.MaxPool2d(kernel_size=2)

        # upsampling
        self.up_concat3 = unetUp(filters[3], filters[2], self.is_deconv, n_concat=self.nslices+1)
        self.up_concat2 = unetUp(filters[2], filters[1], self.is_deconv, n_concat=self.nslices+1)
        self.up_concat1 = unetUp(filters[1], filters[0], self.is_deconv, n_concat=self.nslices+1)
    ...
    def forward(self, inputs):
        ...
        for n in range(self.nslices):
            conv1_0 = self.conv1(inputs[:, n:n+1, :, :])  # output: 64*512*512
            ...
            conv1_.append(conv1_0)
            conv2_.append(conv2_0)
            conv3_.append(conv3_0)
            maxpooling_.append(maxpool3_0)

        conv4 = self.conv4(torch.cat(maxpooling_, 1))  # 512*64*64

        up4 = self.aspp(conv4)
        up3 = self.up_concat3(up4, torch.cat(conv3_, 1))  # 256*128*128
        up2 = self.up_concat2(up3, torch.cat(conv2_, 1))  # 128*256*256
        up1 = self.up_concat1(up2, torch.cat(conv1_, 1))  # 64*512*512
