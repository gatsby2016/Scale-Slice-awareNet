...
class msacUNet(nn.Module):
    def __init__(self, in_channels=1, n_classes=2, feature_scale=1, is_deconv=True, is_batchnorm=True):
        ...
        self.aspp = _ASPP(512, 512, [1, 2, 3, 5, 8], norm_layer=nn.BatchNorm2d, norm_kwargs=None)
        # downsampling
        ...
        self.conv1a = unetConv2(self.in_channels, filters[0], self.is_batchnorm)
        self.conv2a = unetConv2(filters[0], filters[1], self.is_batchnorm)
        self.conv3a = unetConv2(filters[1], filters[2], self.is_batchnorm)

        self.conv1b = unetConv2(self.in_channels, filters[0], self.is_batchnorm)
        self.conv2b = unetConv2(filters[0], filters[1], self.is_batchnorm)
        self.conv3b = unetConv2(filters[1], filters[2], self.is_batchnorm)
    ...
    def forward(self, inputs):
        ...
        conv1_0 = self.conv1a(inputs[:, 0:1, :, :])  # output: 64*512*512
        maxpool1_0 = self.maxpool(conv1_0)  # 64*256*256
        conv2_0 = self.conv2a(maxpool1_0)  # 128*256*256
        maxpool2_0 = self.maxpool(conv2_0)  # 128*128*128
        conv3_0 = self.conv3a(maxpool2_0)  # 256*128*128
        maxpool3_0 = self.maxpool(conv3_0)  # 256*64*64

        conv1_2 = self.conv1b(inputs[:, 2:, :, :])  # output: 64*512*512
        maxpool1_2 = self.maxpool(conv1_2)  # 64*256*256
        conv2_2 = self.conv2b(maxpool1_2)  # 128*256*256
        maxpool2_2 = self.maxpool(conv2_2)  # 128*128*128
        conv3_2 = self.conv3b(maxpool2_2)  # 256*128*128
